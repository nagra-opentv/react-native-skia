...
void RSkComponentParagraph::OnPaint(SkCanvas *canvas) {
  ...
  if(parent) {
      parent->expectedAttachmentCount += buildParagraph(data.attributedString,
      ...
      if(!parent->expectedAttachmentCount || (parent->expectedAttachmentCount == parent->currentAttachmentCount)) {
            paragraph->layout(parent->getFrameSize().width);
            paragraph->paint(canvas, parent->getFrameOrigin().x, parent->getFrameOrigin().y);
      }
      ...
      paraBuilder = std::static_pointer_cast<ParagraphBuilder>(std::make_shared<ParagraphBuilderImpl>(paraStyle,collection_));
      expectedAttachmentCount = buildParagraph(data.attributedString, props.paragraphAttributes, true, paraBuilder);
      currentAttachmentCount = 0;
      ...
      if(!expectedAttachmentCount) {
          ...
          paragraph->layout(frameSize.width);
          paragraph->paint(canvas, framePoint.x, framePoint.y);
      }
   }
